<template>
  ...
  >
    ...
    >
      <Label
        id="title"
        :text="'choose_promotion_piece' | L"
        horizontalAlignment="center"
        :fontSize="cellSize * 0.5"
      />
      <StackLayout
        orientation="horizontal"
        @tap="commitPromotion('q')"
...
const loadPiecePicture = (refShortcut) => {
  piecesPictures[refShortcut] = ImageSource.fromFileSync(
    path.join(
      knownFolders.currentApp().path,
      "components/chessboard/chess_vectors/" + refShortcut + ".png"
    )
  );
};
for (let shortcut of [
  ...
  methods: {
    ...
    pieceImageShortcutAtRankFile(rank, file) {
      let isTheMovingPiece =
        this.dndOriginRank === rank && this.dndOriginFile === file;
      ...
      switch (piece.type) {
        case "p":
          imageBase = piece.color === "b" ? "pd" : "pl";
          break;
        case "n":
          imageBase = piece.color === "b" ? "nd" : "nl";
          break;
        case "b":
          imageBase = piece.color === "b" ? "bd" : "bl";
          break;
        case "r":
          imageBase = piece.color === "b" ? "rd" : "rl";
          break;
        case "q":
          imageBase = piece.color === "b" ? "qd" : "ql";
          break;
        case "k":
          imageBase = piece.color === "b" ? "kd" : "kl";
          break;
      }
    ...
    movedPieceTop() {
      ...
      if (this.promotionDialogOpened) {
        const boardReversedSincePromotionDialog =
          this.boardOrientationBeforePromotionDialog !== this.reversed;
        const realTop = boardReversedSincePromotionDialog
          ? this.size - this.dndMovedPieceTop - this.cellSize
          : this.dndMovedPieceTop;
        return realTop;
      } else {
    ...
    movedPieceLeft() {
      ...
      if (this.promotionDialogOpened) {
        const boardReversedSincePromotionDialog =
          this.boardOrientationBeforePromotionDialog !== this.reversed;
        const realLeft = boardReversedSincePromotionDialog
          ? this.size - this.dndMovedPieceLeft - this.cellSize
          : this.dndMovedPieceLeft;
        return realLeft;
      } else {
    ...
    commitPromotion(typeStr) {
      ...
      this.boardLogic.move({
        from: this.startCellStr,
        to: this.endCellStr,
        promotion: typeStr,
      });
      ...
      lastMoveSan = lastMoveSan.replace(
        "K",
        lastMoveIsWhite ? "\u2654" : "\u265A"
      );
      lastMoveSan = lastMoveSan.replace(
    ...
    reactToTouch(event) {
      ...
      const col = Math.floor(
        (event.getX() - this.halfCellSize) / this.cellSize
      );
      const row = Math.floor(
        (event.getY() - this.halfCellSize) / this.cellSize
      );
      ...
      const pieceAtClickedSquare = this.boardLogic.get(
        rankAndFileToCoordinate(rank, file)
      );

      switch (event.action) {
        case "down":
          const pieceImageShortcut = this.pieceImageShortcutAtRankFile(
            rank,
            file
          );
          ...
          const notOurPiece =
            this.boardLogic.turn() !== pieceAtClickedSquare.color;
        ...
        case "up":
          ...
          this.startCellStr = rankAndFileToCoordinate(
            this.dndOriginRank,
            this.dndOriginFile
          );
          ...
          const moveResult = boardLogicClone.move({
            from: this.startCellStr,
            to: this.endCellStr,
            promotion: "q",
          });
    ...
    gotoHistory(positionIndex) {
      ...
      if (positionIndex === "first") {
        ...
        this.boardLogic.load(
          this.playedMoves[this.historyCursorIndex].positionFen
        );
    ...
    _drawCoordinates(canvas) {
      ...
      for (let col of [0, 1, 2, 3, 4, 5, 6, 7]) {
        ...
        const top = this.cellSize * 0.4;
        const bottom = this.cellSize * 8.9;
    ...
    _drawCells(canvas) {
      ...
      for (let row of [0, 1, 2, 3, 4, 5, 6, 7]) {
        for (let col of [0, 1, 2, 3, 4, 5, 6, 7]) {
          ...
          if (this.dndDestFile === file || this.dndDestRank === rank)
            color = "green";
          if (this.dndOriginFile === file && this.dndOriginRank === rank)
            color = "red";
          ...
          canvas.drawRect(
            createRect(x, y, this.cellSize, this.cellSize),
            paint
          );
        }
      }
    ...
    _drawPlayerTurn(canvas) {
      ...
      canvas.drawArc(
        createRect(x, x, this.halfCellSize, this.halfCellSize),
        360,
        360,
        true,
        paint
    ...
    _drawLastMoveArrow(canvas) {
      ...
      const realOriginCol = this.reversed
        ? 7 - this.lastMove.origin.file
        : this.lastMove.origin.file;
      const realOriginRow = this.reversed
        ? this.lastMove.origin.rank
        : 7 - this.lastMove.origin.rank;

      const realDestCol = this.reversed
        ? 7 - this.lastMove.dest.file
        : this.lastMove.dest.file;
      const realDestRow = this.reversed
        ? this.lastMove.dest.rank
        : 7 - this.lastMove.dest.rank;
      ...
      const edge1StopX =
        baseStopX + this.cellSize * Math.cos(edge1AngleRad) * 0.6;
      const edge1StopY =
        baseStopY + this.cellSize * Math.sin(edge1AngleRad) * 0.6;

      const edge2StartX = baseStopX;
      const edge2StartY = baseStopY;
      const edge2StopX =
        baseStopX + this.cellSize * Math.cos(edge2AngleRad) * 0.6;
      const edge2StopY =
        baseStopY + this.cellSize * Math.sin(edge2AngleRad) * 0.6;
    ...
    _commitComputerMove(moveData) {
      const moveNumber = parseInt(this.boardLogic.fen().split(" ")[5]);
      this.boardLogic.move({
        from: moveData.startCellStr,
        to: moveData.endCellStr,
